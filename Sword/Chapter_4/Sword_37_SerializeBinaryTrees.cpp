/**************************************************
	> File Name:  Sword_37_SerializeBinaryTrees.cpp
	> Author:     Leuckart
	> Time:       2020-02-19 14:47
**************************************************/
...
BinaryTreeNode *Deserialize(istream &stream)
{
	...
	if (ReadStream(stream, &val))
	{
		...
		pRoot->m_nValue = val;
		pRoot->m_pLeft = Deserialize(stream);
		pRoot->m_pRight = Deserialize(stream);
	}
	return pRoot;
}
...
void Test(const char *testName, const BinaryTreeNode *pRoot)
{
	...
	BinaryTreeNode *pNewRoot = Deserialize(fileIn2);
	fileIn2.close();
