/**************************************************
	> File Name:  Sword_32_03_PrintTreesInZigzag.cpp
	> Author:     Leuckart
...
void PrintInZigzag(BinaryTreeNode *pRoot)
{
	...
	while (!S1.empty() || !S2.empty())
	{
		if (S1.empty())
		{
			while (!S2.empty())
			{
				BinaryTreeNode *pTmp = S2.top();
				S2.pop();
				printf("%d\t", pTmp->m_nValue);

				if (pTmp->m_pRight)
					S1.push(pTmp->m_pRight);
				if (pTmp->m_pLeft)
					S1.push(pTmp->m_pLeft);
			}
		}
		else
		{
			while (!S1.empty())
			{
				BinaryTreeNode *pTmp = S1.top();
				S1.pop();
				printf("%d\t", pTmp->m_nValue);

				if (pTmp->m_pLeft)
					S2.push(pTmp->m_pLeft);
				if (pTmp->m_pRight)
					S2.push(pTmp->m_pRight);
			}
		}
		printf("\n");
	}
