/**************************************************
	> File Name:  Sword_27_MirrorOfBinaryTree.cpp
	> Author:     Leuckart
	> Time:       2020-02-18 03:13
**************************************************/
...
void MirrorIteratively(BinaryTreeNode *pRoot)
{
	...
	queue<BinaryTreeNode *> qs;
	qs.push(pRoot);

	while (!qs.empty())
	{
		BinaryTreeNode *pCur = qs.front();
		qs.pop();

		BinaryTreeNode *pTmp = pCur->m_pLeft;
		pCur->m_pLeft = pCur->m_pRight;
		pCur->m_pRight = pTmp;

		if (pCur->m_pLeft)
			qs.push(pCur->m_pLeft);
		if (pCur->m_pRight)
			qs.push(pCur->m_pRight);
	}
}
