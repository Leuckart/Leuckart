/**************************************************
	> File Name:  Sword_54_KthNodeInBST.cpp.cpp
	> Author:     Leuckart
	> Time:       2020-02-27 02:53
**************************************************/
...
const BinaryTreeNode *KthNodeRecursive(const BinaryTreeNode *const pRoot, unsigned int &idx)
{
	if (pRoot == nullptr)
		return nullptr;

	const BinaryTreeNode *pLeft = KthNodeRecursive(pRoot->m_pLeft, idx);
	if (pLeft)
		return pLeft;
	...
	const BinaryTreeNode *pRight = KthNodeRecursive(pRoot->m_pRight, idx);
	if (pRight)
		return pRight;
...
const BinaryTreeNode *KthNode(const BinaryTreeNode *const pRoot, unsigned int idx)
{
	if (pRoot == nullptr || idx <= 0)
		return nullptr;

	return KthNodeRecursive(pRoot, idx);
}
